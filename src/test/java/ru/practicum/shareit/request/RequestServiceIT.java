...
public class RequestServiceIT {
    ...
    @Test
    void addNewRequest() {
        UserDto addedUser = userService.create(userDto);
        requestService.create(addedUser.getId(), requestDto);

        List<ItemRequestDtoOut> actualRequests = requestService.findUserRequests(addedUser.getId());

        assertEquals("request description", actualRequests.get(0)
                .getDescription());
    }
